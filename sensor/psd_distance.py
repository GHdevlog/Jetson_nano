...
if __name__ == "__main__":
    ...
    while True:
        adc_data = analog_read(1)  # 채널 1에서 ADC 데이터 읽기
        # 읽어온 데이터에서 유효한 12비트 데이터 추출
        psd = ((adc_data[1] & 0xf) << 8 | adc_data[2])
        volt = psd * 3.3 / 4096
        
        if volt > 0:  # volt 값이 0보다 큰 경우에만 계산 진행
            distance = 26.549 * math.pow(volt, -1.2091)
            print('distance : %d cm' % distance)  # 추출한 데이터 출력
        else:
            print('Invalid volt value: %f' % volt)
